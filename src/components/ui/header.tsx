    ...
    return (
        <header className={cn("w-full flex flex-row justify-between items-center px-4 sm:px-8 py-5", isHome && 'absolute top-0 left-0')}>
            ...
            <Link href='/' className="flex flex-row items-center gap-2 sm:gap-4">
                ...
                />
                <span className={cn("font-galada h-fit text-2xl sm:text-3xl lg:text-3xl leading-none", isHome && 'text-white')}>AR Rahah</span>
            </Link>
            {/* Nav Link */}
            <HeaderNav />
            {/* Buttons */}
            <ButtonLink href='/contact-us' className={cn("hidden lg:flex", isHome && 'bg-white/30 text-white border-white hover:border-black')}>
